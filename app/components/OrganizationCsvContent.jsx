...
var Head = React.createClass({
  render: function(){
    return (
      <thead>
        <tr>{this.props.columns.map(column => <th key={column}>{column}</th>)}<th></th></tr>
      </thead>
    );
  }
});
...
var Row = React.createClass({
  render: function(){
    return(
      <tr>
        {this.props.columns.map(h => <td key={h}>{this.props.data[h]}</td>)}<td></td>
      </tr>
    );
  }
});

var OrganizationCsvContent = React.createClass({
    render: function(){
...
module.exports = OrganizationCsvContent;
